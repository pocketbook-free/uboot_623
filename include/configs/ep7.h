...
#define CONFIG_FASTBOOT_TRANSFER_BUF     0x70000000
#define CONFIG_FASTBOOT_TRANSFER_BUF_SIZE 0x4000000 /* 90M byte */

#define CONFIG_ANDROID_DIAGS_PARTITION_MMC 1
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"kernel_addr=0x70800040\0"		\
		\
		"bootargs_SD=setenv bootargs console=ttymxc0,115200  fastboot_dev=mmc0 ip=none root=/dev/mmcblk0p2 rootwait\0" \
		"bootargs_MMC=setenv bootargs console=ttymxc0,115200  fastboot_dev=mmc2 ip=none root=/dev/mmcblk1p2 rootwait\0" \
		\
		"load_kernel_SD=mmc read 0 ${kernel_addr} 0x800 0x2000\0" \
		"load_kernel_MMC= mmc read 2 ${kernel_addr} 0x800 0x2000\0" \
		\
        "fatload_uboot_sd=fatload mmc 0:1 ${uboot_addr} u-boot.bin; mmc write 0 ${uboot_addr1} 0x2 0x200\0"  \
        "fatload_kernel_sd=fatload mmc 0:1 ${kernel_addr} uImage; mmc write 0 ${kernel_addr} 0x800 0x2000\0" \
		\
        "prg_uboot_sd=run fatload_uboot_sd\0"  \
        "prg_kernel_sd=run fatload_kernel_sd\0" \
		\
		"bootcmd_SD=run bootargs_SD; run load_kernel_SD; bootm ${kernel_addr}\0"   \
		"bootcmd_MMC=run bootargs_MMC; run load_kernel_MMC; bootm ${kernel_addr}\0" \
