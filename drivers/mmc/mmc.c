...
//	printf("enter ===%s\n", __func__);
	...
	if (err){
		printf("MMC/SD send %hd command error %d\n", cmd.cmdidx, err);
		return err;
	}
...
//		return err;
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		err = mmc_send_ext_csd(mmc, ext_csd);
		if (!err && (ext_csd[192] >= 2) && csize==0xFFF) {
...
void mmc_print_desc(struct mmc *mmc)
{
	printf("block_dev's lun is %d\n", mmc->block_dev.lun);
	printf("block_dev's blksz is %d\n", mmc->block_dev.blksz);
	printf("mmc's capacity is %llu\n", mmc->capacity);
	printf("mmc's high capacity is %d\n", mmc->high_capacity);
	printf("mmc's read block len is %d\n", mmc->read_bl_len);
	printf("mmc's lba is  %llu\n", mmc->block_dev.lba);
	printf("mmc's clock is  %ld\n", mmc->clock);
	printf("mmc's host_caps is  %ld\n", mmc->host_caps);
	printf("mmc's write bl len is  %ld\n", mmc->write_bl_len);
	printf("mmc bus width %0x\n", mmc->bus_width);
}
