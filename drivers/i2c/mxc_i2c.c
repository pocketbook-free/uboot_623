...
/*------------------------- Modify for CW2015 ---------------------------------*/
//	i2c_read(0x36,0x04,1,&soc,1);
//	soc = soc/2;
	if (i2c_addr(0x36, 0x02, 1)) {
		//printf("Can't detect Max17043, try CW2015\n");
                i2c_read(0x62,0x04,1,&soc,1);   //add for cw2015
        } else {
                i2c_read(0x36,0x04,1,&soc,1);   //add for max17043
                soc = soc/2;
	}
...
static int do_i2c_test(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	u16 vol = 0;
	u8 vol_msb = 0,vol_lsb=0;
	u32 temp = 0;
	PAD_I2C1_SCL =0x10;
	PAD_I2C1_SDA = 0x10;
	PULL_PAD_I2C1_SCL &= ~(0x1<<7);// Pull Up/Down Disable	SCL, SDA
...
/*------------------------ Modify for CW2015 --------------------------------- */
#if 0
	i2c_read(0x36,0x02,1,&vol_msb,1);// read max17043 voltage
	i2c_read(0x36,0x03,1,&vol_lsb,1);// read max17043 voltage
	vol_lsb = (vol_lsb>>4) & 0xff;                           
	vol = (vol_msb<<4) | vol_lsb;
	temp = (uint)(vol * 5 / 4);                 
	printf("Now MAX17043's voltage is :  %d mv\n",temp);
#endif
	if (i2c_addr(0x36, 0x02, 1)) {
		printf("Can't detect Max17043, try CW2015\n");
		i2c_read(0x62,0x02,1,&vol_msb,1);// read cw2015 voltage
		i2c_read(0x62,0x03,1,&vol_lsb,1);// read cw2015 voltage
		vol = (vol_msb<<8) | vol_lsb;
		temp = (uint)(vol *312 / 1024);
	} else {
		i2c_read(0x36,0x02,1,&vol_msb,1);// read max17043 voltage
        	i2c_read(0x36,0x03,1,&vol_lsb,1);// read max17043 voltage
        	vol_lsb = (vol_lsb>>4) & 0xff;                           
...
U_BOOT_CMD(
		i2c_test, 1, 0, do_i2c_test,
		"i2c_test   -i2c ......... i2c  test proccess\n ",
		NULL
		);
